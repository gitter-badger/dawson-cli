...
const argv = yargs
  ...
  .command('dev', 'Runs a development server proxying assets (from /) and API Gateway (from /prod)', () =>
    yargs
      ...
      .default('skip-chmod', false)
      .describe('skip-chmod', 'Do not run chmod -Rf a+rw .dawson-dist after installing dependencies (RTFM)')
      .boolean('overwrite-proxy-role')
      .default('overwrite-proxy-role', false)
      .describe('overwrite-proxy-role', 'Deletes and re-creates the IAM Role used by the development proxy')
      .boolean('skip-babelrc-validation')
      .default('skip-babelrc-validation', false)
      .describe('skip-babelrc-validation', 'Do not attempt to validate .babelrc file against recommended settings')
      .describe('verbose', 'Verbose output')
